...
def next_move(track, trains):
    ...
    for (x, y, direction, intersection_direction) in trains[:]:
        ...
        if (train_at(target_x, target_y, trains) and direction in ['>', 'v']):
            kill_list.append((target_x, target_y))
        elif train_at(target_x, target_y, new_trains):
            kill_list.append((target_x, target_y))
    ...
    return [(x, y, d, i) for (x, y, d, i) in new_trains if (x, y) not in kill_list]

def first_crash_location(track):
    trains = get_train_locations(track)
    track = remove_trains(track)
    while len(trains) > 1: 
        trains = next_move(track, trains)
    return trains[0][0:2]
